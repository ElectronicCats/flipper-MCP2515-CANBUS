...
void callback_input_sender_options(void* context, uint32_t index) {
  ...
  switch (index) {
    ...
    case SET_DATA:
      scene_manager_set_scene_state(app->scene_manager,
                                    app_scene_input_text_option, SET_DATA);
      scene_manager_next_scene(app->scene_manager, app_scene_input_text_option);
      break;
...
void callback_sender_options(VariableItem* item) {
  ...
  uint8_t selected_index =
      variable_item_list_get_selected_item_index(app->varList);
...
void app_scene_sender_on_enter(void* context) {
  ...
  // 0
  item = variable_item_list_add(app->varList, "SEND MESSAGE", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);

  // 1
  item = variable_item_list_add(app->varList, "Choose Id", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  variable_item_set_current_value_text(item, "SEARCH");

  // 2
  item = variable_item_list_add(app->varList, "Id", 0, callback_sender_options,
                                app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 3
  item = variable_item_list_add(app->varList, "DLC", 9, callback_sender_options,
                                app);
  ...
  // 4
  item = variable_item_list_add(app->varList, "Request", 2,
                                callback_sender_options, app);
  ...
  // 5
  item = variable_item_list_add(app->varList, "CLIC TO SET DATA", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);

  // 6
  item = variable_item_list_add(app->varList, "BYTE [0]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 7
  item = variable_item_list_add(app->varList, "BYTE [1]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 8
  item = variable_item_list_add(app->varList, "BYTE [2]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 9
  item = variable_item_list_add(app->varList, "BYTE [3]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 10
  item = variable_item_list_add(app->varList, "BYTE [4]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 11
  item = variable_item_list_add(app->varList, "BYTE [5]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 12
  item = variable_item_list_add(app->varList, "BYTE [6]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  // 13
  item = variable_item_list_add(app->varList, "BYTE [7]", 0,
                                callback_sender_options, app);
  variable_item_set_current_value_index(item, 0);
  ...
  variable_item_list_set_enter_callback(app->varList,
                                        callback_input_sender_options, app);
...
void app_scene_send_message_on_enter(void* context) {
  ...
  widget_add_string_element(app->widget, 65, 20, AlignCenter, AlignCenter,
                            FontPrimary, "Wait to send it Message...");

  view_dispatcher_switch_to_view(app->view_dispatcher, ViewWidget);

  app->thread =
      furi_thread_alloc_ex("Sender_on_work", 1024, sender_on_work, app);
  ...
  if (event.type == SceneManagerEventTypeCustom) {
    switch (event.event) {
      case SEND_OK:
        widget_reset(app->widget);
        widget_add_string_element(app->widget, 65, 20, AlignCenter, AlignCenter,
                                  FontPrimary, "MESSAGE SEND OK");
        break;

      case SEND_ERROR:
        widget_reset(app->widget);
        widget_add_string_element(app->widget, 65, 20, AlignCenter, AlignCenter,
                                  FontPrimary, "MESSAGE SEND ERROR");
        break;
...
void app_scene_warning_log_on_enter(void* context) {
  ...
  widget_add_string_element(app->widget, 65, 20, AlignCenter, AlignBottom,
                            FontPrimary, "W A R N I N G");

  widget_add_string_element(app->widget, 65, 40, AlignCenter, AlignBottom,
                            FontSecondary, "First go to the Sniffing option");
  widget_add_string_element(app->widget, 65, 50, AlignCenter, AlignBottom,
                            FontSecondary, "in the menu and get the Id's");
...
void app_scene_id_list_on_enter(void* context) {
  ...
  for (uint8_t i = 0; i < app->num_of_devices; i++) {
    ...
    submenu_add_item(app->submenu, furi_string_get_cstr(app->text), i,
                     menu_ids_callback, app);
  }
...
void input_byte_sender_callback(void* context) {
  ...
  app->frame_to_send->canId =
      app->sender_id_compose[3] | (app->sender_id_compose[2] << 8) |
      (app->sender_id_compose[1] << 16) | (app->sender_id_compose[0] << 24);
  scene_manager_handle_custom_event(app->scene_manager, ReturnEvent);
}
...
void app_scene_input_text_on_enter(void* context) {
  ...
  switch (state) {
    case SET_ID:
      byte_input_set_result_callback(scene, input_byte_sender_callback, NULL,
                                     app, app->sender_id_compose, 4);
      byte_input_set_header_text(app->input_byte_value, "SET ADDRESS");
      break;

    case SET_DATA:
      byte_input_set_result_callback(scene, input_byte_sender_callback, NULL,
                                     app, app->frame_to_send->buffer, 8);
    ...
    default:
      if (state > 5) {
        byte_input_set_result_callback(
            scene, input_byte_sender_callback, NULL, app,
            &(app->frame_to_send->buffer[state - 6]), 1);

        furi_string_reset(app->text);
        furi_string_cat_printf(app->text, "SET BYTE [%lu]", state - 6);
        byte_input_set_header_text(app->input_byte_value,
                                   furi_string_get_cstr(app->text));
      }
      break;
  }
