...
void typical_menu_callback(void* context, uint32_t index) {
  ...
  scene_manager_set_scene_state(app->scene_manager,
                                app_scene_obdii_typical_codes_option, index);
...
void app_scene_obdii_typical_codes_on_enter(void* context) {
  ...
  submenu_add_item(app->submenu, "Vehicle Speed", 1, typical_menu_callback,
                   app);

  view_dispatcher_switch_to_view(app->view_dispatcher, SubmenuView);
  scene_manager_set_scene_state(app->scene_manager,
                                app_scene_obdii_typical_codes_option, 0);
}
...
static int32_t obdii_thread_on_work(void* context) {
  ...
  uint32_t option = scene_manager_get_scene_state(
      app->scene_manager, app_scene_obdii_typical_codes_option);
