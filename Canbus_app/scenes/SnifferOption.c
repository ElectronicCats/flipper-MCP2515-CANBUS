...
char* sequential_file_resolve_path_address(Storage* storage,
                                           uint32_t address,
                                           const char* dir,
                                           const char* prefix,
                                           const char* extension) {
  ...
  do {
    if (snprintf(file_path, sizeof(file_path), "%s/%s_0x%lx_%u_%u_%i_%d.%s",
                 dir, prefix, address, day, month, year, file_index,
                 extension) < 0) {
      return NULL;
    }
    file_index++;
...
void save_data_on_log(App* app) {
  strcpy(app->log_file_path,
         sequential_file_resolve_path(app->storage, PATHLOGS, "Log", "log"));
  if (app->log_file_path != NULL) {
    if (storage_file_open(app->log_file, app->log_file_path, FSAM_WRITE,
                          FSOM_CREATE_ALWAYS)) {
...
void save_address_data_on_log(App* app) {
  ...
  strcpy(app->log_file_path, sequential_file_resolve_path_address(
                                 app->storage, can_id, PATHLOGS, "L", "log"));
  if (app->log_file_path != NULL) {
    if (storage_file_open(app->log_file, app->log_file_path, FSAM_WRITE,
                          FSOM_CREATE_ALWAYS)) {
      app->log_file_ready = true;
...
void draw_box_text(App* app) {
  ...
  furi_string_cat_printf(app->text, "ADDR: %lx DLC: %u \n",
                         app->frameArray[app->sniffer_index].canId,
                         app->frameArray[app->sniffer_index].data_lenght);

  for (uint8_t i = 0; i < (app->frameArray[app->sniffer_index].data_lenght);
       i++) {
    furi_string_cat_printf(app->text, "[%u]:  %x ", i,
                           app->frameArray[app->sniffer_index].buffer[i]);
  }
      ...
      if (!condition) {
        if (frame.canId == app->frameArray[app->sniffer_index].canId) {
          ...
          if (app->log_file_ready && (app->save_logs == OnlyAddress)) {
            write_data_on_file(app->frameArray[app->sniffer_index],
                               app->log_file, app->times[app->sniffer_index]);
          }
        }
      }
...
void app_scene_sniffing_on_enter(void* context) {
  ...
  if (condition) {
    app->thread =
        furi_thread_alloc_ex("SniffingWork", 1024, worker_sniffing, app);
  ...
  if (scene_manager_get_scene_state(app->scene_manager,
                                    app_scene_sniffing_option) == 1) {
    scene_manager_previous_scene(app->scene_manager);
    scene_manager_set_scene_state(app->scene_manager, app_scene_sniffing_option,
                                  0);
  }
...
bool app_scene_device_no_connected_on_event(void* context,
