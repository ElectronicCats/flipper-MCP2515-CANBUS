...
static const char* bitratesValues[] = {"125KBPS", "250KBPS", "500KBPS",
                                       "1000KBPS"};
...
void app_scene_settings_on_enter(void* context) {
  ...
  // Second Item
  item =
      variable_item_list_add(app->varList, "Clock", 0, callback_options, app);
  variable_item_set_current_value_index(item, 0);
  variable_item_set_current_value_text(item, clockValues[currentClock]);

  // Third Item
  item = variable_item_list_add(app->varList, "Save LOGS?", 3, callback_options,
                                app);
